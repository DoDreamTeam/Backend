...
class BookCommentServiceTest {
    ...
    void getCommentList_ShouldReturnPageOfBookCommentResponse() {
        ...
        // When
        Page<BookCommentResponse> commentsPage = bookCommentService.getCommentList(pageable, book.getId(), isSortByLikes, user);
        ...
        assertEquals(comment1.getId(), commentsPage.getContent().get(0).getId()); // Ensure the comments are sorted correctly
        assertEquals(comment2.getId(), commentsPage.getContent().get(1).getId());
    }
    ...
    void getCommentList_ShouldThrowBaseException_WhenBookIdNotFound() {
        ...
        BaseException exception = assertThrows(BaseException.class, () -> {
            bookCommentService.getCommentList(PageRequest.of(0, 5), nonExistentBookId, false, user);
        });
...
//    }
    ...
    @DisplayName("댓글 삭제 시 존재하지 않을 때 예외 발생")
    @Test
